...
export const Experience = ({
  ...
  // 0: Laptop, 1: Left Router, 2: Center Router, 3: Right Router, 4: Right Laptop
  ...
  // Position flags ke hisaab se packet step sync (TCP laptop reset / left router)
  useEffect(() => {
    if (isLayerComplete) {
      setCurrentStep(0);
  ...
  const getHeaderRotation = () => {
    ...
    if (isAtRightLaptop) return [0, -1 * (Math.PI / 2), 0];
  ...
  const handleNextStep = () => {
    // Round 0 aur Central Router (Step 2) par Next Step block rahega
    if (currentStep === 2 && round === 0) {
      return;
    }
